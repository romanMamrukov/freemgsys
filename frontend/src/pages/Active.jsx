...
export default function ActivePage() {
  ...
  const minutes = useMemo(() => elapsedMinutes(task, now), [task, now]);

  useEffect(() => {
    if (!task) return;
    setHours((elapsedMinutes(task) / 60).toFixed(2));
  ...
  const finish = (event) => {
    ...
    completeTask(task.id, {
      totalMinutes: Math.max(0, Number(hours) || 0) * 60,
      comment,
    });
  };
        ...
        <div className="active-layout">
          ...
          <form className="panel completion-panel" onSubmit={finish}>
            ...
            <label className="field">
              ...
              <input type="number" min="0" step="0.01" value={hours} onChange={(event) => setHours(event.target.value)} required />
              <small>You can adjust the tracked time before completing the task.</small>
            </label>
            <label className="field">
